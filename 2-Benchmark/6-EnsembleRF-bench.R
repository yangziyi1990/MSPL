##----------------------------------
##  Code Version 1.0
##  This real data code for Random Forest embeded in the Ensemble-based framework.
##  Ensemble_RF
...
ensemblePanels <- lapply(colon_train_data, function(i){
  RF.colon <- randomForest(x = i, y = factor(colon_train_lable), importance = TRUE, ntree = 10)
})

ensembleValiPredction <- mapply(function(fit,x){
  valprediction <- predict(fit,x)
}, fit = ensemblePanels, x = colon_train_data)

ensembleTestPredction <- mapply(function(fit,x){
  tsprediction <- predict(fit,x,type="response",s="lambda.min")
}, fit = ensemblePanels, x = colon_test_data)

select.feature.colon.idx <- list()
select.feature.colon.value <- list()
select.fearure.colon.name <- list()

for(i in 1:length(ensemblePanels)){
  select.feature.colon.idx[[i]] <- which(ensemblePanels[[i]]$importance[,4]!=0)
  select.feature.colon.value[[i]] <- ensemblePanels[[i]]$importance[which(ensemblePanels[[i]]$importance[,4]!=0),4]
  select.fearure.colon.name[[i]] <- colnames(colon_train_data[[i]])[select.feature.colon.idx[[i]]]
}

ensemblefeature.colon <- list("feature.idx" = select.feature.colon.idx,
                              "feature.vaule" = select.feature.colon.value,
...
results.colon <- list("valpredmatrix" = ensembleValiPredction, "evlpredmatrix" = ensembleTestPredction, 
                      "feature" = ensemblefeature.colon, "ensenmblePanels" = ensemblePanels)
...
ensemblePanels <- lapply(gbm_train_data, function(i){
  RF.gbm <- randomForest(x = i, y = factor(gbm_train_lable), importance = TRUE, ntree = 10)
})

ensembleValiPredction <- mapply(function(fit,x){
  valprediction <- predict(fit,x)
}, fit = ensemblePanels, x = gbm_train_data)

ensembleTestPredction <- mapply(function(fit,x){
  tsprediction <- predict(fit,x,type="response",s="lambda.min")
}, fit = ensemblePanels, x = gbm_test_data)

select.feature.gbm.idx <- list()
select.feature.gbm.value <- list()
select.fearure.gbm.name <- list()

for(i in 1:length(ensemblePanels)){
  select.feature.gbm.idx[[i]] <- which(ensemblePanels[[i]]$importance[,4]!=0)
  select.feature.gbm.value[[i]] <- ensemblePanels[[i]]$importance[which(ensemblePanels[[i]]$importance[,4]!=0),4]
  select.fearure.gbm.name[[i]] <- colnames(gbm_train_data[[i]])[select.feature.gbm.idx[[i]]]
}

ensemblefeature.gbm <- list("feature.idx" = select.feature.gbm.idx,
                            "feature.vaule" = select.feature.gbm.value,
...
results.gbm <- list("valpredmatrix" = ensembleValiPredction, "evlpredmatrix" = ensembleTestPredction, 
                    "feature" = ensemblefeature.gbm, "ensenmblePanels" = ensemblePanels)
...
ensemblePanels <- lapply(kidney_train_data, function(i){
  RF.kidney <- randomForest(x = i, y = factor(kidney_train_lable), importance = TRUE, ntree = 10)
})

ensembleValiPredction <- mapply(function(fit,x){
  valprediction <- predict(fit,x)
}, fit = ensemblePanels, x = kidney_train_data)

ensembleTestPredction <- mapply(function(fit,x){
  tsprediction <- predict(fit,x,type="response",s="lambda.min")
}, fit = ensemblePanels, x = kidney_test_data)

select.feature.kidney.idx <- list()
select.feature.kidney.value <- list()
select.fearure.kidney.name <- list()

for(i in 1:length(ensemblePanels)){
  select.feature.kidney.idx[[i]] <- which(ensemblePanels[[i]]$importance[,4]!=0)
  select.feature.kidney.value[[i]] <- ensemblePanels[[i]]$importance[which(ensemblePanels[[i]]$importance[,4]!=0),4]
  select.fearure.kidney.name[[i]] <- colnames(kidney_train_data[[i]])[select.feature.kidney.idx[[i]]]
}

ensemblefeature.kidney <- list("feature.idx" = select.feature.kidney.idx,
                               "feature.vaule" = select.feature.kidney.value,
...
results.kidney <- list("valpredmatrix" = ensembleValiPredction, "evlpredmatrix" = ensembleTestPredction, 
                       "feature" = ensemblefeature.kidney, "ensenmblePanels" = ensemblePanels)
...
ensemblePanels <- lapply(lung_train_data, function(i){
  RF.lung <- randomForest(x = i, y = factor(lung_train_lable), importance = TRUE, ntree = 10)
})

ensembleValiPredction <- mapply(function(fit,x){
  valprediction <- predict(fit,x)
}, fit = ensemblePanels, x = lung_train_data)

ensembleTestPredction <- mapply(function(fit,x){
  tsprediction <- predict(fit,x,type="response",s="lambda.min")
}, fit = ensemblePanels, x = lung_test_data)

select.feature.lung.idx <- list()
select.feature.lung.value <- list()
select.fearure.lung.name <- list()

for(i in 1:length(ensemblePanels)){
  select.feature.lung.idx[[i]] <- which(ensemblePanels[[i]]$importance[,4]!=0)
  select.feature.lung.value[[i]] <- ensemblePanels[[i]]$importance[which(ensemblePanels[[i]]$importance[,4]!=0),4]
  select.fearure.lung.name[[i]] <- colnames(lung_train_data[[i]])[select.feature.lung.idx[[i]]]
}

ensemblefeature.lung <- list("feature.idx" = select.feature.lung.idx,
                             "feature.vaule" = select.feature.lung.value,
...
results.lung <- list("valpredmatrix" = ensembleValiPredction, "evlpredmatrix" = ensembleTestPredction, 
                     "feature" = ensemblefeature.lung, "ensenmblePanels" = ensemblePanels)
